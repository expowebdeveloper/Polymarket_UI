...
import { fetchViewAllLeaderboards, fetchPaginatedLeaderboard } from '../services/api';
...
function mapEntryToRow(entry: LeaderboardEntry, index: number) {
  const name = entry.name || entry.pseudonym || `Trader ${entry.rank || index + 1}`;
  const handle = entry.wallet_address
    ? `@${entry.wallet_address.slice(0, 6)}...${entry.wallet_address.slice(-4)}`
    : `@trader${entry.rank || index + 1}`;
  ...
  return {
    ...
    volume: formatVolume(entry.total_stakes || 0),
...
// Main
// --------------------
export default function PolyratingLeaderboardPreview() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setError(null);
      try {
        if (location.pathname === '/leaderboard/all') {
          // Map frontend rankBy to backend sort_by
          let sortBy = 'final_score';
          switch (rankBy) {
            case "Score": sortBy = 'final_score'; break;
            case "Win Rate": sortBy = 'win_rate'; break;
            case "ROI": sortBy = 'roi'; break;
            case "PNL": sortBy = 'total_pnl'; break;
            case "Total Volume": sortBy = 'total_stakes'; break;
            case "Rewards": sortBy = 'final_score'; break;
            default: sortBy = 'final_score';
          }

          const offset = (page - 1) * itemsPerPage;
          const response = await fetchPaginatedLeaderboard(itemsPerPage, offset, sortBy, true);

          const mappedRows = (response.entries || []).map((entry: any, index: number) =>
            mapEntryToRow(entry, offset + index)
          );

          setRowsData(mappedRows);
          setTotalPages(Math.ceil((response.count || 0) / itemsPerPage));
          // Set data to satisfy type checks if needed, or leave it null for this path
          // But we need data for other things maybe? Not really if we use rowsData.
          setData(response as any); // minimal compatibility

        } else {
          // Legacy behavior for other routes
          const result = await fetchViewAllLeaderboards('all', 'PNL', 100, 0);
          setData(result);
        }
      } catch (err) {
    ...
    loadData();
  }, [location.pathname, page, rankBy]);

  // Client-side fallback logic (Legacy)
  const allEntries = useMemo(() => {
    if (location.pathname === '/leaderboard/all') return []; // specific optimization
    ...
    switch (rankBy) {
      ...
      case "PNL":
        ...
        break;
      case "Total Volume":
        entries = (data.leaderboards.final_score || []).slice().sort((a, b) => (b.total_stakes || 0) - (a.total_stakes || 0));
        break;
      case "Rewards":
        entries = data.leaderboards.final_score || [];
        break;
    ...
    return entries.map(mapEntryToRow);
  }, [data, rankBy, location.pathname]);
  ...
  const rows = useMemo(() => {
    ...
    if (rankBy === "Score") sorted.sort((a, b) => b.score - a.score);
    else if (rankBy === "Win Rate") sorted.sort((a, b) => b.winRate - a.winRate);
    else if (rankBy === "ROI") sorted.sort((a, b) => b.roi - a.roi);
    else if (rankBy === "PNL") sorted.sort((a, b) => b.pnl - a.pnl);
    else if (rankBy === "Total Volume") sorted.sort((a, b) => b.volumeRaw - a.volumeRaw);
    else if (rankBy === "Rewards") sorted.sort((a, b) => b.reward - a.reward);

    const startIndex = (page - 1) * itemsPerPage;
    return sorted.slice(startIndex, startIndex + itemsPerPage);
  }, [allEntries, query, rankBy, page, location.pathname, rowsData]);

  // Update total pages for client-side fallback
  useEffect(() => {
    if (location.pathname !== '/leaderboard/all') {
      const count = allEntries.filter((r) =>
        `${r.name} ${r.handle}`.toLowerCase().includes(query.trim().toLowerCase())
      ).length;
      setTotalPages(Math.ceil(count / itemsPerPage));
    }
  }, [allEntries, query, location.pathname]);
