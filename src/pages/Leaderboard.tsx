...
export function Leaderboard() {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            let orderBy: 'PNL' | 'VOL' | 'ROI' | 'WIN_RATE' | 'SCORE' = 'SCORE';
            switch (rankBy) {
            ...
            const transformedRows: TableRow[] = response.entries.map((entry: LeaderboardEntry) => ({
                rank: entry.rank,
                name: entry.name || entry.pseudonym || "Unknown",
                handle: entry.pseudonym ? `@${entry.pseudonym}` : (entry.wallet_address ? `${entry.wallet_address.slice(0, 8)}...` : "—"),
                score: entry.final_score || 0,
                winRate: entry.win_rate || 0,
                volume: formatMoney(entry.total_stakes || 0).replace('+', ''),
                volumeRaw: entry.total_stakes || 0,
                roi: entry.roi || 0,
                pnl: entry.total_pnl || 0,
                reward: Math.floor(Math.random() * 50) + 10, // Mock reward
                profileImage: entry.profile_image,
                userTag: entry.user_tag || null,
    ...
    const displayRows = useMemo(() => {
        ...
        return rows.filter((r) =>
            `${r.name} ${r.handle} ${r.walletAddress}`.toLowerCase().includes(query.trim().toLowerCase())
        );
    }, [rows, query]);
                ...
                <div className="mx-auto max-w-6xl">
                    ...
                    <div className="mb-6">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="hidden items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 text-slate-300 px-3 py-2 text-sm shadow-sm md:flex">
                                    ...
                                    {query ? (
                                        <button
                                            onClick={() => setQuery("")}
                                            className="rounded-md p-1 text-slate-400 hover:bg-slate-800 dark:hover:bg-slate-700 dark:hover:text-slate-200"
                    ...
                    <div className="mb-6 flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                className="rounded-xl border border-slate-800 bg-slate-900 text-slate-200 px-5 py-3 text-sm font-semibold shadow-sm transition hover:shadow"
                                onClick={() => {
                                    ...
                                    {loading ? (
                                        ...
                                        <tr>
                                            <td colSpan={9} className="py-8 text-center text-red-500">
                                                {error}
                                                <button onClick={fetchData} className="ml-4 text-blue-500 underline">Retry</button>
                                            </td>
                                        </tr>
                                        ...
                                        displayRows.map((r) => (
                                            <tr
                                                key={r.walletAddress || r.rank}
                                                className="border-b border-slate-800/60 hover:bg-slate-800/40 last:border-b-0"
                                            >
                                                ...
                                                <td className="px-6 py-5">
                                                    <div className="flex items-center gap-3">
                                                        {r.profileImage ? (
                                                            <img src={r.profileImage} alt="" className="h-9 w-9 rounded-full object-cover" />
                                                        ) : (
                                                ...
                                                <td className={`px-6 py-5 text-sm font-semibold ${r.roi >= 0 ? "text-emerald-300" : "text-red-300"}`}>
                                                    {r.roi >= 0 ? "+" : ""}{r.roi.toFixed(1)}%
                                                </td>

                                                <td className={`px-6 py-5 text-sm font-semibold ${r.pnl >= 0 ? "text-emerald-300" : "text-red-300"}`}>
                                                    {formatMoney(r.pnl)}
                                                </td>

                                                <td className="px-6 py-5">
                                                    <TagPill label={r.rank <= 3 ? "Elite Whale" : "Whale"} userTag={r.userTag} />
                                                </td>
